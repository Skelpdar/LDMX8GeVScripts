...
class sampleContainer:
    def __init__(self, fn,maxEvts,trainFrac,isBkg,iseBkg,iseSig):
        ...
        #self.tin.Add(fn)
        self.tfile = r.TFile(fn);
        self.tin = self.tfile.Get("BDTree")
        #self.tin.Print()
...
if __name__ == "__main__":
    ...
    #parser.add_option('--max_evt', dest='max_evt',type="int",  default=20000, help='Max Events to load')
    ...
    print(eventContainer.test_y,preds)
    testPreds = []
    testPreds.append(eventContainer.test_y)
    testPreds.append(preds)
    print(testPreds)
    testPreds=np.asarray(testPreds)
    np.save(options.out_name+'_'+str(adds)+"/"+options.out_name+'_'+str(adds)+'_validation_preds.npy',testPreds)
