/*
 *Evaluates the BDT score of ldmx-sw 1.7.0 data
...
void evalLongitudinal(TString infilename, TString outfilename, bool noise = false, double noiseFactor = 1.25, bool completeOutput = false, bool reconstruct = false){
    ...
    //Seg2 variables
    double ele68TotalEnergy_long0;
    double ele68TotalEnergy_long1;
    double ele68TotalEnergy_long2;
    double photon68TotalEnergy_long0;
    double photon68TotalEnergy_long1;
    double photon68TotalEnergy_long2;
    double outside68TotalEnergy_long0;
    double outside68TotalEnergy_long1;
    double outside68TotalEnergy_long2;
    double outside68NHits_long0;
    double outside68NHits_long1;
    double outside68NHits_long2;
    double outside68Zmean_long0;
    double outside68Zmean_long1;
    double outside68Zmean_long2;
    double outside68Zstd_long0;
    double outside68Zstd_long1;
    double outside68Zstd_long2;
    ...
    if(completeOutput){
        ...
        auto ele68TotalEnergy_long0Branch = outtree->Branch("ele68TotalEnergy_long0",&ele68TotalEnergy_long0);
        auto ele68TotalEnergy_long1Branch = outtree->Branch("ele68TotalEnergy_long1",&ele68TotalEnergy_long1);
        auto ele68TotalEnergy_long2Branch = outtree->Branch("ele68TotalEnergy_long2",&ele68TotalEnergy_long2);
        auto photon68TotalEnergy_long0Branch = outtree->Branch("photon68TotalEnergy_long0",&photon68TotalEnergy_long0);
        auto photon68TotalEnergy_long1Branch = outtree->Branch("photon68TotalEnergy_long1",&photon68TotalEnergy_long1);
        auto photon68TotalEnergy_long2Branch = outtree->Branch("photon68TotalEnergy_long2",&photon68TotalEnergy_long2);
        auto outside68TotalEnergy_long0Branch = outtree->Branch("outside68TotalEnergy_long0",&outside68TotalEnergy_long0);
        auto outside68TotalEnergy_long1Branch = outtree->Branch("outside68TotalEnergy_long1",&outside68TotalEnergy_long1);
        auto outside68TotalEnergy_long2Branch = outtree->Branch("outside68TotalEnergy_long2",&outside68TotalEnergy_long2);
        auto outside68NHits_long0Branch = outtree->Branch("outside68NHits_long0",&outside68NHits_long0);
        auto outside68NHits_long1Branch = outtree->Branch("outside68NHits_long1",&outside68NHits_long1);
        auto outside68NHits_long2Branch = outtree->Branch("outside68NHits_long2",&outside68NHits_long2);
        auto outside68Zmean_long0Branch = outtree->Branch("outside68Zmean_long0",&outside68Zmean_long0);
        auto outside68Zmean_long1Branch = outtree->Branch("outside68Zmean_long1",&outside68Zmean_long1);
        auto outside68Zmean_long2Branch = outtree->Branch("outside68Zmean_long2",&outside68Zmean_long2);
        auto outside68Zstd_long0Branch = outtree->Branch("outside68Zstd_long0",&outside68Zstd_long0);
        auto outside68Zstd_long1Branch = outtree->Branch("outside68Zstd_long1",&outside68Zstd_long1);
        auto outside68Zstd_long2Branch = outtree->Branch("outside68Zstd_long2",&outside68Zstd_long2);
    }
    ...
    //for(int i=0; i<1; i++){
        ...
        //Variables for Seg2
        ele68TotalEnergy_long0 = 0;
        ele68TotalEnergy_long1 = 0;
        ele68TotalEnergy_long2 = 0;
        photon68TotalEnergy_long0 = 0;
        photon68TotalEnergy_long1 = 0;
        photon68TotalEnergy_long2 = 0;
        outside68TotalEnergy_long0 = 0;
        outside68TotalEnergy_long1 = 0;
        outside68TotalEnergy_long2 = 0;
        outside68NHits_long0 = 0;
        outside68NHits_long1 = 0;
        outside68NHits_long2 = 0;
        outside68Zmean_long0 = 0;
        outside68Zmean_long1 = 0;
        outside68Zmean_long2 = 0;
        outside68Zstd_long0 = 0;
        outside68Zstd_long1 = 0;
        outside68Zstd_long2 = 0;
            ...
            for(int k=0; k < ecalHits->GetEntriesFast(); k++){
               ...
               // std::cout << mcid_val << ":" <<mcid_index_slow << ":" << mcid_index << std::endl;
                ...
                if(distanceEle < ir_radius){
                    ...
                    if(layer < 6){
                        ele68TotalEnergy_long0 += hitE;
                    }
                    else if(layer < 16){
                        ele68TotalEnergy_long1 += hitE;
                    }
                    else{
                        ele68TotalEnergy_long2 += hitE;
                    }
                }
                ...
                if(distancePhoton < ip_radius && distancePhoton > 0){
                    ...
                    if(layer < 6){
                        photon68TotalEnergy_long0 += hitE;
                    }
                    else if(layer < 16){
                        photon68TotalEnergy_long1 += hitE;
                    }
                    else{
                        photon68TotalEnergy_long2 += hitE;
                    }
                }
        ...
        */
        ...
        //Calculate "outside" variables for the Seg2 BDT

        for(int k = 0; k < outside68HitPositionsX.size(); k++){
            if(outside68HitPositionsLayer.at(k) < 6){
                outside68TotalEnergy_long0 += outside68HitPositionsE.at(k);
                outside68NHits_long0 += 1;
                outside68Zmean_long0 += layerZs.at(outside68HitPositionsLayer.at(k))*outside68HitPositionsE.at(k);
            }
            else if(outside68HitPositionsLayer.at(k) < 16){
                outside68TotalEnergy_long1 += outside68HitPositionsE.at(k);
                outside68NHits_long1 += 1;
                outside68Zmean_long1 += layerZs.at(outside68HitPositionsLayer.at(k))*outside68HitPositionsE.at(k);
            }
            else{
                outside68TotalEnergy_long2 += outside68HitPositionsE.at(k);
                outside68NHits_long2 += 1;
                outside68Zmean_long2 += layerZs.at(outside68HitPositionsLayer.at(k))*outside68HitPositionsE.at(k);
            }
        }

        if(outside68NHits_long0 > 0){
            outside68Zmean_long0 /= outside68TotalEnergy_long0;
        }
        if(outside68NHits_long1 > 0){
            outside68Zmean_long1 /= outside68TotalEnergy_long1;
        }
        if(outside68NHits_long2 > 0){
            outside68Zmean_long2 /= outside68TotalEnergy_long2;
        }

        for(int k = 0; k < outside68HitPositionsX.size(); k++){
            if(outside68HitPositionsLayer.at(k) < 6){
                outside68Zstd_long0 += pow(layerZs.at(outside68HitPositionsLayer.at(k))-outside68Zmean_long0,2)*outside68HitPositionsE.at(k); 
            }
            else if(outside68HitPositionsLayer.at(k) < 16){
                outside68Zstd_long1 += pow(layerZs.at(outside68HitPositionsLayer.at(k))-outside68Zmean_long1,2)*outside68HitPositionsE.at(k); 
            }
            else{
                outside68Zstd_long2 += pow(layerZs.at(outside68HitPositionsLayer.at(k))-outside68Zmean_long2,2)*outside68HitPositionsE.at(k); 
            }   
        }
        if(outside68NHits_long0 > 0){
            outside68Zstd_long0 = TMath::Sqrt(outside68Zstd_long0/outside68TotalEnergy_long0);
        }
        if(outside68NHits_long1 > 0){
            outside68Zstd_long1 = TMath::Sqrt(outside68Zstd_long1/outside68TotalEnergy_long1);
        }
        if(outside68NHits_long2 > 0){
            outside68Zstd_long2 = TMath::Sqrt(outside68Zstd_long2/outside68TotalEnergy_long2);
        }

        //Recalculate all other BDT features if noise is added (or if told to)
